...
- **`develop`**: 
  ...
  - 1인 프로젝트에 맞추어 별도의 기능(feature) 브랜치 없이 **모든 로컬 개발 작업의 커밋과 PR은 `develop` 브랜치에 직접 수행**하여 세부 내용을 관리합니다.
...
* **PR Process**: 1인 프로젝트이므로 기능 추가 및 수정 사항은 `develop`에 커밋하고, 일정 단위 작업이 마무리되면 `develop`에서 `main`으로 PR을 보내어 병합합니다. 모든 작업 이력은 PR 본문과 커밋 메시지로 관리합니다.
