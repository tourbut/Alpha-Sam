---
name: manage-skills
description: 기존 스킬을 동적으로 탐색하고, 새 스킬을 생성하거나 기존 스킬을 업데이트한 뒤 SKILLS.md를 관리합니다.
...
| `verify-frontend-style` | 프론트엔드 코드 스타일 검증 | `frontend/src/**/*.svelte`, `frontend/src/**/*.ts`, `frontend/src/**/*.js` |
| `verify-git-rules` | 커밋 및 브랜치 규칙 검증 | (전체 로컬 Git 변경 사항) |
...
6. **SKILLS.md 자체** — SKILLS.md의 변경은 문서 업데이트이며, 검증이 필요한 코드 패턴이 아님
