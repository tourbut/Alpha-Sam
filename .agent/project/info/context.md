...
### 8. 에이전트 전용 인터페이스 (Agent-Centric Interface) - 기획 중
- **개요**: LLM 기반 에이전트가 UI DOM 파싱에 따른 토큰 낭비를 없애고 100% 정확도로 시스템을 조작할 수 있도록 하는 하이브리드 아키텍처.
- **초경량 진입점 (`/agent/login`)**:
  - CSS 꾸밈 요소나 중첩 태그를 모두 배제하고, 순수 시맨틱 HTML 폼으로 구성된 로그인 화면.
  - 인증 성공 시 화면에 **JWT Access Token**과 **필수 API 명세(엔드포인트, Payload 구조)** 를 순수 텍스트(Plain text)로 즉시 렌더링.
- **REST API 제어 전환 (Headless Control)**:
  - 에이전트는 진입점에서 토큰을 획득한 직후 브라우징(UI 제어)을 중단.
  - 획득한 토큰을 활용해 백엔드 API를 직접 호출하여 포트폴리오/자산/트랜잭션 CRUD 작업을 수행.
...
| `/agent/login` | **API 토큰 및 명세 획득을 위한 에이전트 전용 경량 진입점** |
| `/portfolios` | 포트폴리오 목록 |
