...
## 4. Agent-Centric Interface (v2.2.0)

### Hybrid Control Flow
- **Role**: AI 에이전트를 위한 초경량 웹 진입점 및 API 통신 제어 로직.
- **Rules**:
  - `/agent/login`은 스타일링 없이 순수 시맨틱 HTML로만 구성하여 DOM 파싱 토큰 낭비를 최소화한다.
  - 에이전트 로그인 성공 시 JWT 토큰과 필수 API 명세(JSON)를 Plain Text 형식으로 응답값에 렌더링한다.
  - 에이전트는 진입점에서 토큰을 획득한 후 추가 브라우저 컨트롤을 중단하고 즉시 REST API 제어로 전환(Headless Control)해야 한다.
  - 에이전트가 생성한 포트폴리오, 자산, 거래 내역은 모두 기존 회원과 동일한 `owner_id` 제약 및 도메인 룰을 따른다.
